...
package com.example.flowcodelabmodule
...
import com.example.flowcodelabmodule.util.CacheOnSuccess
import com.example.flowcodelabmodule.utils.ComparablePair
...
class PlantRepository private constructor(
    ...
    private fun List<Plant>.applySort(customSorOrder: List<String>): List<Plant> {
        return sortedBy { plant ->
            val positionForItem = customSorOrder.indexOf(plant.plantId).let { order ->
    ...
    fun getPlantsWithGrowZoneFlow(growZoneNumber: GrowZone): Flow<List<Plant>> =
        plantDao.getPlantsWithGrowZoneNumberFlow(growZoneNumber.number)
            .map { plantList ->
                val sortOrderFomNetwork = plantsListSortOrderCache.getOrAwait()
                val nextValue = plantList.applySort(sortOrderFomNetwork)
                nextValue
            }
